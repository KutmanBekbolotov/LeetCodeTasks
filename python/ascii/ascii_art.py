...
class ArtConverter:
    ...
    def run(self):
        frame_delay = 1 / self.rec_fps  # Время на один кадр
        while True:
            frame_start = pg.time.get_ticks() / 1000  # Текущее время в секундах
            ...
            # Ждём, чтобы соблюсти правильный FPS
            elapsed = (pg.time.get_ticks() / 1000) - frame_start
            if elapsed < frame_delay:
                pg.time.wait(int((frame_delay - elapsed) * 1000))
